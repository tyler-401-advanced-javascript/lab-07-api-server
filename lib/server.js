...
function putHandler(type, req, res) {
  if (req.params.id && (req.body.name || req.body.category)) {
    ...
    const found = db[type].find(el => el.id === req.params.id);
    if (req.body.name) found.name = req.body.name;
    if (req.body.category) found.category = req.body.category;
    res.status(202).json(found);
  } else {
   throw new Error(`Cant update: id or name missing.`);
  }
}
...
function deleteHandler(type, req, res) {
  ...
  if (db[type].find(cat => cat.id === idParam)) {
    ...
    throw new Error(`Couldn't find that category`);
  }
  res.status(200).json({});
}
...
//heres my error handler. Hope express can find it way down here! 
